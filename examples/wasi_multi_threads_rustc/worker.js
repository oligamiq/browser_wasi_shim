...
self.onmessage = async (e) => {
	...
	rewrite = new SharedObject.SharedObjectRef("rewrite").proxy();
	...
	// rewrite /helloworld/Cargo.toml /helloworld/src/main.rs
	await term.writeln(
		`\n$${blueText} rewrite /helloworld/Cargo.toml /helloworld/src/main.rs${resetText}`,
	);
	await rewrite("/helloworld/Cargo.toml", "/helloworld/src/main.rs");
	...
	// cargo run --manifest-path /helloworld/Cargo.toml --jobs 1 --target wasm32-wasip1 --release
	await term.writeln(
		`\n$${blueText} cargo run --manifest-path /helloworld/Cargo.toml --jobs 1 --target wasm32-wasip1 --release${resetText}`,
	);
	try {
		await cargo(
			"run",
	...
	// 	}
	// } else {
	// 	console.error("error: could not exec the linker");
	// }
