...
async function runOnMainThread() {
  console.log("--- Testing WASI (non-threads) on Main Thread (should fallback to busy-wait) ---");
  const wasi = new WASI(
    ["sleep.wasm", "500"],
    [],
    [
      new OpenFile(new File([])),
      ConsoleStdout.lineBuffered((msg) => console.log(`[Main WASI stdout] ${msg}`)),
      ConsoleStdout.lineBuffered((msg) => console.warn(`[Main WASI stderr] ${msg}`)),
    ]
  );
...
async function runInWorker() {
  ...
  const farm = new WASIFarm(
    new OpenFile(new File([])),
    ConsoleStdout.lineBuffered((msg) => console.log(`[Worker WASI stdout] ${msg}`)),
    ConsoleStdout.lineBuffered((msg) => console.warn(`[Worker WASI stderr] ${msg}`)),
    []
  );
