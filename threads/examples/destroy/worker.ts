...
import { WASIFarm, DestroyerHandle } from "../../src";
...
self.onmessage = (e) => {
  if (e.data.destroyer) {
    console.log("[Worker] Received destroyer handle, calling destroy()...");
    const destroyer = DestroyerHandle.init_self(e.data.destroyer);
    destroyer.destroy();
